...
func (c *categoryAPI) GetCategory(w http.ResponseWriter, r *http.Request) {
	...
	newData, err := c.categoryService.GetCategories(r.Context(), int(idLogin))
	if err != nil {
...
func (c *categoryAPI) CreateNewCategory(w http.ResponseWriter, r *http.Request) {
	...
	i, err := c.categoryService.StoreCategory(r.Context(), &cat)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
...
func (c *categoryAPI) DeleteCategory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		log.Println("delete category", err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("invalid user id"))
		return
	}
	...
	err = c.categoryService.DeleteCategory(r.Context(), int(catID))
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		log.Println(err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("error internal server"))
